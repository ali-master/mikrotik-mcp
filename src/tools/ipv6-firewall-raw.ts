...
export const ipv6FirewallRawTools: ToolModule = [
  defineTool({
    ...
    async handler(a, ctx) {
      ctx.info(
        `Creating IPv6 firewall raw rule: chain=${a.chain}, action=${a.action}`,
      );
      ...
      const count = await executeMikrotikCommand(
        "/ipv6 firewall raw print detail count-only",
        ctx,
      );
      const c = count.trim();
  ...
  defineTool({
    ...
    annotations: READ,
    description:
      "Gets detailed information about a specific IPv6 firewall raw rule.",
    inputSchema: {
  ...
  defineTool({
    ...
    description:
      "Updates an existing IPv6 firewall raw rule. " +
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const result = await executeMikrotikCommand(
        `/ipv6 firewall raw remove ${a.rule_id}`,
        ctx,
      );
      if (looksLikeError(result))
  ...
  defineTool({
    ...
    annotations: WRITE_IDEMPOTENT,
    description:
      "Moves an IPv6 firewall raw rule to a different position in the chain.",
    inputSchema: {
      rule_id: z.string(),
    ...
    async handler(a, ctx) {
      ctx.info(
        `Moving IPv6 firewall raw rule: rule_id=${a.rule_id} to position ${a.destination}`,
      );
