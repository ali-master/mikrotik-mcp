...
async function runGetLogs(o: GetLogsOptions, ctx: ToolContext): Promise<string> {
  ...
  ctx.info(`Getting logs with filters: topics=${o.topics}, action=${o.action}, time=${o.time_filter}`);
...
export const logTools: ToolModule = [
  defineTool({
    ...
    title: "Get Logs",
    annotations: READ,
    description: "Gets logs from the MikroTik device with optional topic, time, and message filters.",
    inputSchema: {
  ...
  defineTool({
    ...
    annotations: READ,
    description: "Gets logs for a specific topic/facility (system, dhcp, interface, firewall, etc.).",
    inputSchema: {
      topic: z.string(),
      time_filter: z.string().optional(),
      limit: z.number().int().optional(),
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      return runGetLogs({ topics, message_filter, time_filter: a.time_filter, limit: a.limit }, ctx);
    },
  }),
  ...
  defineTool({
    ...
    async handler(_a, ctx) {
      ...
      for (const topic of topics) {
        const count = await executeMikrotikCommand(`/log print count-only where topics~"${topic}"`, ctx);
        const c = count.trim();
  ...
  defineTool({
    ...
    title: "Export Logs",
    annotations: READ,
    description: "Exports logs to a file on the MikroTik device with optional topic and time filters.",
    inputSchema: {
