...
async function addRoute(a: AddRouteArgs, ctx: Parameters<typeof executeMikrotikCommand>[1]): Promise<string> {
  ...
  if (t) {
    if (t.includes("*") || /^\d+$/.test(t)) {
      ...
      return details.trim() ? `Route added successfully:\n\n${details}` : `Route added with ID: ${result}`;
    }
  ...
  return details.trim() ? `Route added successfully:\n\n${details}` : "Route addition completed but unable to verify.";
}
...
export const routeTools: ToolModule = [
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      if (a.routing_mark !== undefined) {
        cmd.raw(a.routing_mark === "" ? "!routing-mark" : `routing-mark=${quoteValue(a.routing_mark)}`);
      }
      ...
      if (a.vrf_interface !== undefined) {
        cmd.raw(a.vrf_interface === "" ? "!vrf-interface" : `vrf-interface=${quoteValue(a.vrf_interface)}`);
      }
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const t = result.trim();
      if (t) {
        if (t.includes("*") || /^\d+$/.test(t)) return `Blackhole route added successfully. ID: ${result}`;
  ...
  defineTool({
    ...
    async handler(_a, ctx) {
      ...
      const active = await executeMikrotikCommand("/ip route print count-only where active=yes", ctx);
      const dynamic = await executeMikrotikCommand("/ip route print count-only where dynamic=yes", ctx);
      const staticCount = await executeMikrotikCommand("/ip route print count-only where static=yes", ctx);
      const disabled = await executeMikrotikCommand("/ip route print count-only where disabled=yes", ctx);
