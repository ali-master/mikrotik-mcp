...
export const routingTableTools: ToolModule = [
  ...
  defineTool({
    ...
    annotations: READ,
    description:
      "Gets detailed information about a specific routing table definition by name.",
  ...
  defineTool({
    ...
    inputSchema: {
      name: z
        .string()
        .describe("Unique table name, referenced by routes and routing rules"),
      fib: z
        .boolean()
        .default(true)
        .describe("Install routes into the FIB (forwarding plane)"),
    ...
    async handler(a, ctx) {
      ...
      if (looksLikeError(result))
        return `Failed to add routing table: ${result}`;
  ...
  defineTool({
    ...
    description:
      "Updates a routing table's fib flag, comment, or disabled state.",
    inputSchema: {
    ...
    async handler(a, ctx) {
      ...
      if (commandUnsupported(result)) return UNSUPPORTED;
      if (looksLikeError(result))
        return `Failed to update routing table: ${result}`;
      const details = await executeMikrotikCommand(
        `/routing table print detail where name="${a.name}"`,
        ctx,
      );
  ...
  defineTool({
    ...
    description:
      "Removes a routing table by name. The built-in `main` table cannot be removed.",
    ...
    async handler(a, ctx) {
      ...
      if (commandUnsupported(result)) return UNSUPPORTED;
      if (looksLikeError(result))
        return `Failed to remove routing table: ${result}`;
