...
export const routingIdTools: ToolModule = [
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      if (commandUnsupported(result)) return UNSUPPORTED;
      return isEmpty(result) ? `Routing ID '${a.name}' not found.` : `ROUTING ID DETAILS:\n\n${result}`;
    },
  }),
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      if (a.select_dynamic_id !== undefined) {
        cmd.raw(a.select_dynamic_id === "" ? "!select-dynamic-id" : `select-dynamic-id=${a.select_dynamic_id}`);
      }
