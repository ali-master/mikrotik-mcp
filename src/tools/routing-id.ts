...
export const routingIdTools: ToolModule = [
  ...
  defineTool({
    ...
    annotations: READ,
    description:
      "Gets detailed information about a specific Router-ID instance by name.",
    inputSchema: {
  ...
  defineTool({
    ...
    inputSchema: {
      ...
      id: z
        .string()
        .optional()
        .describe("Fixed router-id as an IPv4 address, e.g. '10.0.0.1'"),
      select_dynamic_id: z
        .string()
        .optional()
        .describe(
          "Interface/loopback name to auto-derive the id from when `id` is omitted",
        ),
  ...
  defineTool({
    ...
    inputSchema: {
      ...
      select_dynamic_id: z
        .string()
        .optional()
        .describe('Interface/loopback name, or "" to clear'),
      comment: z.string().optional(),
    ...
    async handler(a, ctx) {
      ...
      if (looksLikeError(result))
        return `Failed to update routing ID: ${result}`;
      const details = await executeMikrotikCommand(
        `/routing id print detail where name="${a.name}"`,
        ctx,
      );
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      if (commandUnsupported(result)) return UNSUPPORTED;
      if (looksLikeError(result))
        return `Failed to remove routing ID: ${result}`;
