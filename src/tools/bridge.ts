...
export const bridgeTools: ToolModule = [
  defineTool({
    ...
    inputSchema: {
      ...
      vlan_filtering: z.boolean().default(false).describe("Enable 802.1Q VLAN filtering on the bridge"),
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const result = await executeMikrotikCommand(`/interface bridge print${whereClause(filters)}`, ctx);
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const result = await executeMikrotikCommand(`/interface bridge remove [find name="${a.name}"]`, ctx);
  ...
  defineTool({
    ...
    inputSchema: {
      ...
      pvid: z.number().int().min(1).max(4094).optional().describe("Port VLAN ID for untagged ingress"),
      comment: z.string().optional(),
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const result = await executeMikrotikCommand(`/interface bridge port print${whereClause(filters)}`, ctx);
      return isEmpty(result) ? "No bridge ports found matching the criteria." : `BRIDGE PORTS:\n\n${result}`;
    },
  }),
  ...
  defineTool({
    ...
    inputSchema: { interface: z.string().describe("Interface to remove from the bridge") },
    async handler(a, ctx) {
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const result = await executeMikrotikCommand(`/interface bridge host print${whereClause(filters)}`, ctx);
      return isEmpty(result) ? "No bridge hosts found matching the criteria." : `BRIDGE HOSTS:\n\n${result}`;
    },
  }),
  ...
  defineTool({
    ...
    annotations: WRITE,
    description: "Adds a VLAN entry to a bridge's VLAN table (requires vlan-filtering on the bridge).",
    inputSchema: {
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const result = await executeMikrotikCommand(`/interface bridge vlan print${whereClause(filters)}`, ctx);
      return isEmpty(result) ? "No bridge VLANs found matching the criteria." : `BRIDGE VLANS:\n\n${result}`;
    },
  }),
];
