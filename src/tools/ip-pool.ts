...
async function updatePool(a: UpdatePoolArgs, ctx: Parameters<typeof executeMikrotikCommand>[1]): Promise<string> {
  ...
  const details = await executeMikrotikCommand(`/ip pool print detail where name="${detailsName}"`, ctx);
...
export const ipPoolTools: ToolModule = [
    ...
    async handler(a, ctx) {
      ...
      if (t) {
        if (t.includes("*") || /^\d+$/.test(t)) {
          const details = await executeMikrotikCommand(`/ip pool print detail where name="${a.name}"`, ctx);
          return details.trim() ? `IP pool created successfully:\n\n${details}` : `IP pool created with ID: ${result}`;
        }
        ...
        for (const line of resultLines) {
          ...
          if (line.includes("name=")) {
            ...
            if (nameStart > 5 && nameEnd > nameStart) {
              ...
              if (/^\d+$/.test(usedCount.trim())) outputLines.push(`      used-addresses=${usedCount.trim()}`);
            }
          }
        }
        return `IP POOLS:\n\n${  outputLines.join("\n")}`;
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const result = await executeMikrotikCommand(`/ip pool print detail where name="${a.name}"`, ctx);
      if (isEmpty(result)) return `IP pool '${a.name}' not found.`;

      const usedCount = await executeMikrotikCommand(`/ip pool used print count-only where pool="${a.name}"`, ctx);
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const count = await executeMikrotikCommand(`/ip pool print count-only where name="${a.name}"`, ctx);
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const result = await executeMikrotikCommand(`/ip pool used print${whereClause(filters)}`, ctx);
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      const current = await executeMikrotikCommand(`/ip pool print detail where name="${a.name}"`, ctx);
