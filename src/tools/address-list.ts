...
export const addressListTools: ToolModule = [
  ...
  defineTool({
    ...
    async handler(a, ctx) {
      ...
      return isEmpty(result) ? "No address-list entries found matching the criteria." : `FIREWALL ADDRESS-LISTS:\n\n${result}`;
    },
  }),
  ...
  defineTool({
    ...
    annotations: READ,
    description: "Gets detailed information about a specific address-list entry by its internal id.",
    inputSchema: { entry_id: z.string().describe("Internal entry id, e.g. '*1'") },
    async handler(a, ctx) {
      ...
      return isEmpty(result) ? `Address-list entry '${a.entry_id}' not found.` : `ADDRESS-LIST ENTRY DETAILS:\n\n${result}`;
    },
  }),
